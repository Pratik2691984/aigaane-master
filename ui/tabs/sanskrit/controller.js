...
import { verifyChandasOverlayAudit } from "./chandas/chandas-overlay-verification-engine.js";
...
import { buildNiruktaOverlay, attachNiruktaToGraph } from "./nirukta/nirukta-etymology-engine.js";
import { renderNiruktaOverlay } from "./nirukta/nirukta-etymology-renderer.js";
...
function extractSandhiTokenPairs(tokens) {
  const pairs = [];
...
function extractMorphologyTokens(inputText) {
  const tokens = String(inputText || "")
    .split(/\s+/)
    .filter(Boolean);
  const accepted = [];
...
function renderPrakriyaTraceGraphPanel(inputText = "") {
  ...
  const niruktaOverlay = buildNiruktaOverlay({
    generatedPadas: execution.generatedPadas,
    prakriyaExecution: execution,
    rulefire: linkedRulefire,
    sutraDependency: sutraDependencyGraph,
    semanticOverlay,
    traceGraph,
  });
  const rulefireGraph = attachNiruktaToGraph(
    attachSutraDependenciesToGraph(attachRulefireToGraph(traceGraph, linkedRulefire), sutraDependencyGraph),
    niruktaOverlay,
  );
  ...
  renderNiruktaOverlay(niruktaContainer, niruktaOverlay);
}
...
function isFragmentStanzaMeter(meter) {
...
function renderChandasProsodyPanel(inputText = "") {
  ...
  container.appendChild(inspectionHost);
    const snapshot = createChandasOverlaySnapshot(inspection, {
    sequence: 1,
  });
  ...
  container.appendChild(historyHost);
    ...
    const replay = buildChandasOverlayReplay(history, {
    index: 1,
  });
  ...
  container.appendChild(auditHost);
    const verification = verifyChandasOverlayAudit(audit);

  const verificationHost = document.createElement("div");
  verificationHost.className = "chandas-overlay-verification-host";
  verificationHost.innerHTML = renderChandasOverlayVerification(verification);
  container.appendChild(verificationHost);

    const certification =
    certifyChandasOverlayVerification(verification);

  const certificationHost = document.createElement("div");
  certificationHost.className =
    "chandas-overlay-certification-host";

  certificationHost.innerHTML =
