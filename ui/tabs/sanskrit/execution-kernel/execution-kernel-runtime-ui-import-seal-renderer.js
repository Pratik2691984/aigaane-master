...
function renderRuntimeUiImportSealPanel(record = {}) {
  const entries = Array.isArray(record.importSealEntries)
    ? record.importSealEntries
    : [];

  const body = [
    "Import Status: " + escapeHtml(record.uiImportSealStatus),
    "Import Mode: " + escapeHtml(record.uiImportSealMode),
  ...
  return freeze({
    title: "Runtime UI Import Seal",
    status: entries.length ? "UI_IMPORT_SEAL_READY" : "EMPTY",
    readOnly: true,
    body
  });
}
