...
function renderRuntimeUiCertificationSealPanel(record = {}) {
  const entries = Array.isArray(record.certificationSealEntries)
    ? record.certificationSealEntries
    : [];

  const body = [
    "Certification Status: " + escapeHtml(record.uiCertificationSealStatus),
    "Certification Mode: " + escapeHtml(record.uiCertificationSealMode),
  ...
  return freeze({
    title: "Runtime UI Certification Seal",
    status: entries.length ? "UI_CERTIFICATION_SEAL_READY" : "EMPTY",
    readOnly: true,
    body
  });
}
