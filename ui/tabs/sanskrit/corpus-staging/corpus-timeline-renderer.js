"use strict";

function freeze(v) {
  return Object.freeze(v);
}

function escapeHtml(v) {
  return String(v)
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;");
}

function renderCorpusTimelinePanel(summary = {}) {
  const body = [
    "State: " + escapeHtml(summary.state || "EMPTY"),
    "Valid: " + escapeHtml(Boolean(summary.valid)),
    "Timeline Count: " + escapeHtml(summary.timelineCount || 0),
    "Total Estimated Seconds: " + escapeHtml(summary.totalEstimatedSeconds || 0),
    "Final Completion Percent: " + escapeHtml(summary.finalCompletionPercent || 0),

    "",
    "Preview Only: true",
    "Read Only: true",
    "Timeline Execution Allowed: false",
    "Schedule Execution Allowed: false",
    "Execution Allowed: false",
    "Canonical Write Allowed: false",
    "Promotion Allowed: false",
    "Import Allowed: false"
  ].join("\n");

  return freeze({
    title: "Sanskrit Bulk Corpus Timeline",
    status: summary.valid ? "TIMELINE_READY" : "TIMELINE_BLOCKED",
    readOnly: true,
    body
  });
}

module.exports = {
  renderCorpusTimelinePanel
};