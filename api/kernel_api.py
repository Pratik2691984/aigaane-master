...
try:
    from api.engines.derivation_replay_exporter import DerivationReplayExporter
except ModuleNotFoundError:
...
def derivation_replay_error(message: str) -> HTTPException:
    return HTTPException(
        status_code=400,
        detail={
            "code": "derivation_replay_error",
            "message": message,
        },
    )
...
@app.post("/api/v3/debug/replay/export")
async def debug_replay_export_v3(payload: Dict[str, Any]):
    if "session" not in payload:
        raise derivation_replay_error("session is required.")

    try:
        session = DerivationSession.from_dict(payload["session"])
        replay = derivation_replay_exporter.export_replay(session)
    except (KeyError, TypeError, ValueError) as exc:
        raise derivation_replay_error(str(exc)) from exc
...
@app.get("/api/v3/debug/replay/demo")
async def debug_replay_demo_v3():
    try:
        session = DerivationSession.create(
            input_text="\u0930\u093e\u092e\u0903 \u0905\u0938\u094d\u0924\u093f",
            metadata={
                "source": "debug_replay_demo",
                "semantic_layer": "derivation_replay_export",
            },
        )
        morphology_step = session.add_step(
            engine="Node 3 Morphology",
        ...
        session.add_ambiguity_branch(
            {
                "candidate_id": "demo-branch-primary",
